...
//          .attr('transform', 'translate(' + ((this.options.size / 2)) + ', ' + ((this.options.size / 2) + (this.options.size * 0.15)) + ')');
          .attr('transform', 'translate(' + ((this.options.size / 2)) + ', ' + ((this.options.size) - (this.options.size * 0.05)) + ')');
      }
    }
    if (this.options.scale.enabled) {
  ...
  draw() {
    ...
    function dragInteraction() {
      that.inDrag = true;
      var x = (<any>d3.event).x - (that.options.size / 2);
      var y = (<any>d3.event).y - (that.options.size / 2);
